...
$(document).ready(function(){ 
                ...
                data: {func: 'formatHDD'},
                 success:function(data) {
                     if(data.status == 'OK'){
                        ...
                        openModal({title:modalInfo[0].title,text:modalInfo[0].text,btn:modalInfo[0].btn,input:generateModalInput([["act","format-hdd"]])});
	...
	$('.generate-heartbleed').on('click', function(){
            // here format
            $.ajax({
                type: "POST",
                url: 'ajax.php',
                dataType: "json",
                data: {func: 'generateHeartbleed'},
                 success:function(data) {
                     if(data.status == 'OK'){
                        modalInfo = $.parseJSON(data.msg);
                        openModal({title:modalInfo[0].title,text:modalInfo[0].text,btn:modalInfo[0].btn,input:generateModalInput([["act","generate-heartbleed"]])});
                    }
	        ...
	        if($('body').hasClass('learn')){
	            ...
	            function generateCertificationDiv(opts, lang){
	                ...
	                if($var.hasClass('complete')){
                        ...
                        text = lang.c+'<br><a href="?opt=certification&learn='+opts.cert+'">Read again</a>';
    ...
    if($('body').hasClass('missions')){
        ...
        $('.skip-tutorial').on('click', function(){
            ...
            type: "POST",
            url: "ajax.php",
            data: {func: 'gettext', id:'skip-tutorial'}, 
            success:
                function(data) {
                    ...
                    openModal({title:title,text:text,input:generateModalInput([["act", "skip-tutorial"],["mid", $('.skip-tutorial').attr('value')]]),btn:btn});
