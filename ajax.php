...
// Check for specific POST functions to set loggedOut flag
if (isset($_POST['func'])) {
    switch ($_POST['func']) {
        case 'check-user':
        case 'check-mail':
...
function handleGetBadge($result)
{
    $result['status'] = 'ERROR';
    ...
    $clan = strpos($_SERVER['HTTP_REFERER'], 'clan') !== false;
    ...
    if (!$clan) {
        ...
        $sql = 'SELECT round, dateAdd, COUNT(badgeID) AS total 
                ...
                GROUP BY badgeID';
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':badgeID' => $badgeID, ':userID' => $userID]);
        ...
        $sql = 'SELECT round, dateAdd, COUNT(badgeID) AS total 
                ...
                GROUP BY badgeID';
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':badgeID' => $badgeID, ':clanID' => $userID]);
        $badgeInfo = $stmt->fetch(PDO::FETCH_OBJ);
    ...
    if (!empty($badgeInfo)) {
        ...
        if ($badgeJSON->$badgeID->collectible) {
            if ($badgeInfo->total == 1) {
                ...
                foreach ($data as $dateInfo) {
                    $awardedStr .= substr($dateInfo->dateAdd, 0, -9) . ' - Round #' . $dateInfo->round . '<br/>';
                }
        ...
        if ($clan) {
            ...
            if ($totalPlayers == 0) {
                ...
                $playersStr = '<a href="clan?id=' . $userID . '">' . $user . '</a> and <strong>' . $totalPlayers . '</strong> other clans have this badge.';
            }
        } else {
            $session->newQuery();
            $sql = "SELECT COUNT(*)
            ...
            if ($totalPlayers == 0) {
                ...
                $playersStr = '<a href="profile?id=' . $userID . '">' . $user . '</a> and <strong>' . $totalPlayers . '</strong> other players have this badge.';
            }
        }

        $result['msg'] = json_encode([[
            'title' => $title,
            'text' => '<center><strong>' . $badgeName . '</strong>' . $badgeDesc . '<br/><br/>' . $totalAwarded . $awardedStr . $playersStr . '</center>',
            'btn' => 'OK'
        ]]);
    }
...
function handleGetText($result)
{
    ...
    switch ($id) {
        ...
        case 'certdiv':
            ...
            $certify_paid = _('Get certified for $');
            ...
            $extra = ',"c":"' . $completed . '","c_label":"' . $completed_label . '","c_all":"' . $completed_all . '","take":"' . $take . '","learning":"' . $learning_label . '","cert_free":"' . $certify_free . '","cert_paid":"' . $certify_paid . '","locked":"' . $locked . '"';
        ...
        case 'certbuy':
            ...
            $text = sprintf(_('Are you sure you want to get certified in %s for %s?'), $name, '<font color=\"red\">$' . number_format($p) . '</font>');
            $btn = '<input type=\"submit\" class=\"btn btn-success\" value=\"' . _('Get certified') . '\">';
            break;
        default:
            $result['status'] = 'ERROR';
            break;
...
function handleCertification($result)
{
    ...
    // Perform actions related to certification, for example:
    switch ($certId) {
        case 'start':
            // Add logic to start certification
            $result['status'] = 'OK';
            $result['message'] = 'Certification started.';
            break;
        case 'complete':
            // Add logic to complete certification
            $result['status'] = 'OK';
            $result['message'] = 'Certification completed.';
            break;
        default:
            $result['status'] = 'ERROR';
            break;
...
// Handle bankCloseAcc functionality
function handleBankCloseAcc($result)
{
    ...
    $accinfo = $finances->bankAccountInfo($_SESSION['BANK_ID']);

    if ($accinfo['VALID_ACC'] == '0') {
        $result['msg'] = _('You do not have an account.');
        return $result;
    }
...
// Handle credits functionality
function handleCredits($result)
{
    // Validate if session user and certification ID is set
    if (!isset($_POST['id'])) {
        $result['status'] = 'ERROR';
...
// Handle handlePremium functionality
function handlePremium($result)
{
    // Validate if session user and certification ID is set
    if (!isset($_POST['id'])) {
        $result['status'] = 'ERROR';
...
// Handle deleteDdos functionality
function handleDeleteDdos($result)
{
    // Validate if session user and certification ID is set
    if (!isset($_POST['id'])) {
        $result['status'] = 'ERROR';
...
// Handle bankChangePass functionality
function handleBankChangePass($result)
{
    // Validate if session user and certification ID is set
    if (!isset($_POST['id'])) {
        $result['status'] = 'ERROR';
    ...
    $text = 'Are you sure you want to change your account password? This will cost you $25.00.';
    $btn = '<input id="modal-submit" type="submit" class="btn btn-primary" value="Yes"><a data-dismiss="modal" class="btn" href="#">' . _('Cancel') . '</a>';
...
// Handle buyLicense functionality
function handleBuyLicense($result)
{
    $result['status'] = 'ERROR';
...
function handleGetResearchStats($result)
{
    $result['status'] = 'ERROR';
    ...
    $select = ($round == 'all') ? 'SUM(researchCount) AS researchcount, SUM(moneyResearch) AS moneyresearch' : 'researchCount, moneyResearch';
    $where = ($round == 'all') ? '' : 'ORDER BY id DESC LIMIT 1';
...
function handleGetPartModal($result)
{
    $result['status'] = 'ERROR';
    ...
    if (!isset($opts['part']) || !isset($opts['price']) || !ctype_digit($opts['price'])) {
        $result['status'] = 'ERROR';
        return $result;
    }
...
function handleGetTutorialVirusID($result)
{
    $result['status'] = 'ERROR';
...
function handleGetTutorialFirstVictim($result)
{
    $result['status'] = 'ERROR';
...
function handleGetPlayerLearning($result)
{
    $result['status'] = 'ERROR';
...
function handleGetTotalMoney($result)
{
    $result['status'] = 'ERROR';
...
function handleGetBankAccs($result)
{
    $result['status'] = 'ERROR';
...
function handleManageViruses($result)
{
    $result['status'] = 'ERROR';
...
function handleSearchClan($result)
{
    $result['status'] = 'ERROR';
    ...
    $html = '<li>
                <div class="span12">
                    '._('Type at least two characters').'
                </div>
    ...
    if (strlen($result) > 1) {
        ...
        $session->newQuery();
        $sql = 'SELECT clanID, name, nick, power, slotsUsed FROM clan WHERE name LIKE "%'.$result.'%" OR nick LIKE "%'.$result.'%" LIMIT 10';
...
function fetchWarHistory($result)
{
    $result['status'] = 'ERROR';
    ...
    $warInfo = $pdo->query("SELECT COUNT(*) AS total, idWinner, idLoser, scoreWinner, scoreLoser, startDate, endDate, bounty, round, TIMESTAMPDIFF(DAY, startDate, endDate) AS duration FROM hist_clans_war WHERE id = $result LIMIT 1")->fetch(PDO::FETCH_OBJ);

    if ($warInfo->total !== 1) {
        return ['status' => 'ERROR', 'msg' => 'Sorry, we couldn\'t find the logs of this war.'];
    }

    $myClan = $session['CLAN_ID'];
    ...
    $sql = "SELECT COUNT(*) AS total, attackerClan, victimClan, ddosID FROM clan_ddos_history WHERE warID = $result";
    $ddosHistInfo = $pdo->query($sql)->fetchAll();

    if (sizeof($ddosHistInfo) > 0) {
        foreach ($ddosHistInfo as $ddosHist) {
            if ($warInfo->round == $curRound) {
                $sql = 'SELECT COUNT(*) AS total, attID, vicID, power, servers, date, vicNPC FROM round_ddos WHERE id = '.$ddosHist['ddosid'];
            } else {
                $sql = 'SELECT COUNT(*) AS total, attID, vicID, power, servers, date, vicNPC FROM round_ddos WHERE id = '.$ddosHist['ddosid'].' AND round = '.$ddosHist['round'];
            }
        ...
        $ddosString = 'Ops. We couldn\'t find the logs of this war. Sorry';
    }
...
function handleLoadSoftware($result)
{
    $result['status'] = 'ERROR';
    ...
    while ($softInfo = $data->fetch(PDO::FETCH_OBJ)) {
        ...
        if ($valid == 1) {
            ...
            if ($softInfo->softtype == $prevType) {
                ...
                $return .= '{"text":"' . _($software->int2stringSoftwareType($softInfo->softtype)) . '","children":[{"id":"' . $softInfo->id . '","text":"' . $softName . '"}';
                $times = 0;
...
function handleLoadHistory($result)
{
    $result['status'] = 'ERROR';
