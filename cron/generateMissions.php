...
function generateMissions($level, $total, $pdo){
    ...
    $sqlQuery = "SELECT missions.id FROM missions WHERE missions.status = 1 AND level = '".$level."'";
    $tmp = $pdo->query($sqlQuery)->fetchAll();
    $availableMissions = count($tmp);
    
    //coloca todos os npcs contratantes num array
    $sqlQuery = "SELECT id, npcIP FROM npc WHERE npcType = ".$npcType;
    ...
    while($i < $total){
        ...
        switch($type){
            ...
            case 5: //ddos
                ...
                $sql = "SELECT npc.npcIP FROM hardware INNER JOIN npc ON hardware.userID = npc.id WHERE (hardware.cpu <> '500' OR hardware.hdd <> '1000' OR hardware.ram <> '256' OR hardware.net <> '1') AND hardware.isNPC = '1' AND npc.npcType = '4' AND npc.npcIP <> '".$hirer."' LIMIT 1";
