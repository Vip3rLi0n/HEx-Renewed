...
function dotVersion($softVersion) {
    ...
    switch (strlen($softVersion)) {
        ...
        case '4': //132.8
            ...
            break;
        case '5': //1329.8
            $strEdit = str_split($softVersion, 4);
            break;
        case '6': //12345.8
            $strEdit = str_split($softVersion, 5);
            break;
        case '7': //123456.8
            $strEdit = str_split($softVersion, 6);
            break;
        case '8': //1234567.8
...
while ($pInfo = $data->fetch(PDO::FETCH_OBJ)) {
    ...
    $sqlDropDuplicates = "DELETE FROM hist_users WHERE (user, reputation, age, clanName, timePlaying, hackCount, ddosCount, bitcoinSent, ipResets, moneyEarned, moneyTransfered, moneyHardware, moneyResearch, bestSoft, bestSoftVersion, warezSent, spamSent, profileViews, researchCount) IN (
        SELECT user, reputation, age, clanName, timePlaying, hackCount, ddosCount, bitcoinSent, ipResets, moneyEarned, moneyTransfered, moneyHardware, moneyResearch, bestSoft, bestSoftVersion, warezSent, spamSent, profileViews, researchCount
        FROM (
            SELECT user, reputation, age, clanName, timePlaying, hackCount, ddosCount, bitcoinSent, ipResets, moneyEarned, moneyTransfered, moneyHardware, moneyResearch, bestSoft, bestSoftVersion, warezSent, spamSent, profileViews, researchCount, ROW_NUMBER() OVER (PARTITION BY user, reputation, age, clanName, timePlaying, hackCount, ddosCount, bitcoinSent, ipResets, moneyEarned, moneyTransfered, moneyHardware, moneyResearch, bestSoft, bestSoftVersion, warezSent, spamSent, profileViews, researchCount ORDER BY (SELECT NULL)) AS rn
            FROM hist_users
        ) AS duplicates
        WHERE rn > 1
    )";
    $pdo->query($sqlDropDuplicates);
...
$outputFilePath = "../../database_dump_$dateTime.sql";
...
if ($file) {
    ...
    foreach ($tables as $table) {
        // Retrieve the table structure and data as SQL statements
        $tableStructure = $pdo->query("SHOW CREATE TABLE $table")->fetch(PDO::FETCH_COLUMN);
        $tableData = $pdo->query("SELECT * FROM $table")->fetchAll(PDO::FETCH_ASSOC);

        // Write the table structure to the output file
        fwrite($file, $tableStructure . ";\n\n");

        // Iterate over each row and generate INSERT statements
        foreach ($tableData as $row) {
            $rowValues = implode("', '", array_values($row));
            $insertStatement = "INSERT INTO $table VALUES ('$rowValues');\n";
