...
for userID in cur.fetchall():
    rank += 1
    userID = userID[0]
    cur.execute(f"UPDATE ranking_user SET rank = {rank} WHERE userID = {userID}")
...
for clanID in cur.fetchall():
    rank += 1
    clanID = clanID[0]
    cur.execute(f"UPDATE ranking_clan SET rank = {rank} WHERE clanID = {clanID}")

# software ranking
cur.execute("""
    UPDATE ranking_software 
    INNER JOIN (
        SELECT s.softID, COUNT(i.softID) AS total 
        FROM software_research s 
        LEFT JOIN software_research i ON i.newVersion > s.newVersion AND i.softID IN (
            SELECT softID 
...
for softID in cur.fetchall():
    rank += 1
    softID = softID[0]
    cur.execute(f"UPDATE ranking_software SET rank = {rank} WHERE softID = {softID}")
...
for ddosID in cur.fetchall():
    rank += 1
    ddosID = ddosID[0]
    cur.execute(f"UPDATE ranking_ddos SET rank = {rank} WHERE ddosID = {ddosID}")
