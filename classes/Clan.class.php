        ...
        switch($type){
            ...
            case 'kick':
                ...
                if($bcrypt->verify($pass, $data->password) || $data->password == '0' || $data->password == ''){
                    ...
                    $this->session->addMsg('User kicked.', 'notice');
...
<?php    
    ...
    public function getClanInfo($cid){
        $this->session->newQuery();
        $sql = "SELECT clan.name, clan.clanID AS cid, clan.nick, hist_clans_current.reputation AS power, clan.clanIP, clan_stats.won, clan_stats.lost, clan_stats.pageClicks AS clicks, ranking_clan.rank, (SELECT COUNT(*) FROM clan_users WHERE clan_users.clanID = :cid) AS members FROM clan INNER JOIN clan_stats ON clan_stats.cid = clan.clanID LEFT JOIN ranking_clan ON clan.clanID = ranking_clan.clanID LEFT JOIN hist_clans_current ON hist_clans_current.cid = clan.clanID WHERE clan.clanID = :cid LIMIT 1";
        $data = $this->pdo->prepare($sql);
    ...
    public function show_clanInfo($cid = '')
    {
        ...
        if ($this->clanID != '') {
            ...
            if ($this->clanInfo !== false) {
                ...
                echo 'Failed to retrieve clan information.';
                }
                } else {
...
var uid = <?php echo $this->clanID; ?>;
    ...
    public function acceptRequest($requestID, $requestInfo)
    {
        ...
        // Reset the remote session. Otherwise, the user will find a few bugs for being part of a clan but not having its ID on the user session.
        $this->session->newQuery();
    ...
    public function show_manageSelf(){
        ...
        if($clanInfo->members == 1){
            $class = '';
            $text = _('I am sorry but this feature is not yet implemented. Please contact us if you wish to delete (or leave) your clan. Send an in-game mail to admin or moderator.');
        } else {
            $class = 'red';
