...
class HardwareVPC extends Player {
    ...
    public function getUserClanID($id, $cid) {
        if ($id == '') {
            $id = $_SESSION['id'];
        }
        $this->session->newQuery();
        $sqlQuery2 = "SELECT clanID FROM clan_users WHERE userID = ".$id." LIMIT 1";
        $getClanID = $this->pdo->query($sqlQuery2)->fetch(PDO::FETCH_OBJ);
    }
    ...
    public function showPCTotal($uid, $remote = 0){
        ...
        if($cpu <= 1000){
            ...
            $cpuStr = 'THz';
            $cpu /= 1000000;
        } elseif ($cpu >= 1000000000) {
            $cpuStr = 'PHz';
            $cpu /= 1000000000;
        } elseif ($cpu >= 1000000000000) {
            $cpuStr = 'EHz';
            $cpu /= 1000000000000;
        }
        ...
        if (is_float($hdd)) {
            ...
            if ($hdd >= 1000000000000) {
                $hdd /= 1000000000000;
                $hddStr = 'EXB';
            } elseif ($hdd >= 1000000000){
                $hdd /= 1000000000;
                $hddStr = 'PTB';
            } elseif ($hdd >= 1000000) {
                $hdd /= 1000000;
    ...
    public function getInternetRates($speed)
    {
        ...
        if ($return['download'] >= 10000000) {
            $return['downloadstr'] = round(($return['download'] / 1000000), 2) . ' GB/s';
        } elseif ($return['download'] >= 1000) {
            $return['downloadstr'] = round(($return['download'] / 1000), 2) . ' MB/s';
        } else {
            $return['downloadstr'] = round($return['download'], 2) . ' kB/s';
        }
        
        if ($return['upload'] >= 10000000) {
            $return['uploadstr'] = round(($return['upload'] / 1000000), 2) . ' GB/s';
        } elseif ($return['upload'] >= 1000) {
    ...
    private function formatUnit($value, $baseUnit, $maxUnit)
    {
        $units = array('kB', 'MB', 'GB', 'TB', 'PTB', 'EXB');
        $baseValue = $value;
        $currentUnit = $baseUnit;
        
        while ($baseValue >= 1000 && $currentUnit !== $maxUnit) {
            $baseValue /= 1000;
            $currentUnit = $units[array_search($currentUnit, $units) + 1];
        }
        
        return round($baseValue, 2) . ' ' . $currentUnit;
    }
    ...
    public function showServerInfo($pcInfo, $pcID)
    {
        ...
        <table>
            <tr><td><b>CPU:</b> </td><td><?php echo $this->formatUnit($cpuItens[$pcInfo['CPU']]['POW'], 'Mhz', 'EHz'); ?></td></tr>
            <tr><td><b>RAM:</b> </td><td><?php echo $this->formatUnit($ramItens[$pcInfo['RAM']]['POW'], 'MB', 'EXB'); ?></td></tr>
            <tr><td><b>HDD:</b> </td><td><?php echo $this->formatUnit($hddItens[$pcInfo['HDD']]['POW'], 'MB', 'EXB'); ?></td></tr>
        </table>
    ...
    public function handlePost(){
        ...
        if($type == 'rename'){
            ...
            switch($act){
                ...
                case 'BUY-PC':
                    if ($isClan = 0 && ($_SERVER['QUERY_STRING'] === 'hardware')) {
                    ...
                    if ($isClan = 1 && ($_SERVER['QUERY_STRING'] === 'internet')) {
                    ...
                    // Perform redirect to avoid errors. TEMP FIX ONLY!
                ...
                case 'hdd':    
                    ...
                    if($isClan == 0){
                        if($act == 'net'){
                            ...
                            if(($act == 'ram' && $partID > 3) || $partID > 5){
                                ...
                                if($pcSpec['CPU'] > 4000 && $pcSpec['HDD'] > 10000 && $pcSpec['RAM'] > 2048){
                                    ...
                                    if ($hardwareInfo['CPU'] > 4000 * 4 && $hardwareInfo['HDD'] > 10000 * 4 && $hardwareInfo['RAM'] > 2048 * 4) {
                    ...
                    if ($act == 'hdd' || $act == 'xhd') {
                        ...
                        } elseif ($partArray[$partID]['POW'] > 1000 && $partArray[$partID]['POW'] <= 1000000) {
                            $power = ($partArray[$partID]['POW'] / 1000) . ' GB';
                        } elseif ($partArray[$partID]['POW'] > 1000000 && $partArray[$partID]['POW'] <= 1000000000) {
                            $power = ($partArray[$partID]['POW'] / 1000000) . ' TB';
                        } elseif ($partArray[$partID]['POW'] > 1000000000 && $partArray[$partID]['POW'] <= 1000000000000) {
                            $power = ($partArray[$partID]['POW'] / 1000000000) . ' PTB';
                        } elseif ($partArray[$partID]['POW'] > 1000000000000) {
                            $power = ($partArray[$partID]['POW'] / 1000000000000) . ' EXB';
                        }
                    } elseif ($act == 'cpu') {
                        if ($partArray[$partID]['POW'] < 1000) {
                            ...
                            $power = ($partArray[$partID]['POW'] / 1000000) . ' THz';
                        } elseif ($partArray[$partID]['POW'] > 1000000000 && $partArray[$partID]['POW'] <= 1000000000000) {
                            $power = ($partArray[$partID]['POW'] / 1000000000) . ' PHz';
                        } elseif ($partArray[$partID]['POW'] > 1000000000000) {
                            $power = ($partArray[$partID]['POW'] / 1000000000000) . ' EHz';
                        }
                    } elseif ($act == 'ram') {
                        $power = $partArray[$partID]['POW'] . ' MB';
                    } else {
    ...
    public function store_showPage($specs, $page, $internet = '') {
        ...
        switch ($page) {
            ...
            case 'BUY_PC':    
                ...
                require_once 'Finances.class.php';
                $finances = new Finances();
    ...
    public function addServer($specs, $internet = 0) {
        ...
        if ($internet != 0) {
            ...
            $clanIP = $clanInfo->clanIP;
            $userID = parent::getIDByIP($clanIP, 'NPC');
            $userID = $userID['0']['id'];
            $npc = 1;
        } else {
            $userID = $_SESSION['id'];
        ...
        $this->session->newQuery();
        $sql = "INSERT INTO hardware (userID, name, cpu, net, hdd, ram, isNPC) VALUES ($userID, '".$specs['NAME']."', '".$specs['CPU']."', '', '".$specs['HDD']."', '".$specs['RAM']."', $npc)";
        $this->pdo->query($sql);
    }
...
?>
                                    <div class="hd-usage">
                                        <div class="chart easyPieChart chartpie" data-percent="<?php echo ceil(floatval($porct)); ?>">
...
class LogVPC extends Player {
    ...
    public function listLog($id, $pcType, $local) {
        ...
        if ($valid == '1') {
            ...
            if (count($data) == 0) {
                echo 'No logs entry in database, Please check database!';
            } else {
                $str = ($local == 1) ? 1 : 0;
    ...
    public function addLog($uid, $logText, $npc) {
        ...
        if ($valid == '1') {
            ...
            die("Error while adding log. Contact admin.");
        }
    }
    ...
    public function issetLog($logID) {
        $this->session->newQuery();
        $sql = "SELECT COUNT(*) FROM log WHERE id = :logID";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(array(':logID' => $logID));
