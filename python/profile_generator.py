...
def format(number):
    if isinstance(number, tuple):
        # Handle tuple case separately
        formatted_parts = []
        for part in number:
            if part > 0 and part < 1:
                formatted_parts.append(str(part))
            else:
                formatted_parts.append('%d' % part)
...
def PlayTime(seconds):
    ...
    if days == 0:
        ...
        ret = trans.ngettext('%d hour', '%d hours', int(hrs)) % int(hrs)
        if hrs == 0:
            ret = ''
        elif mins != 0:
            ret += " " + __("and") + " "
        ...
        if mins != 0:
            ret += trans.ngettext('%d minute', '%d minutes', int(mins)) % int(mins)

    else:
        ret = trans.ngettext('%d day', '%d days', int(days)) % int(days)
        if hrs != 0:
            ret += " " + __("and") + " "
            ret += trans.ngettext('%d hour', '%d hours', int(hrs)) % int(hrs)
...
cur.execute("SELECT users.login, users.premium, clan.clanID, clan.name, clan.nick, clan.createdBy, ranking_user.rank, DATEDIFF(CURDATE(), users_stats.dateJoined) AS gameAge, users_stats.exp, users_stats.timeplaying, users_stats.hackCount, users_stats.ddosCount, users_stats.warezSent, users_stats.spamSent, users_stats.ipResets, users_stats.moneyEarned, users_stats.moneyTransfered, users_stats.moneyHardware, users_stats.moneyResearch, users_stats.profileViews, (SELECT COUNT(*) FROM missions_history WHERE missions_history.userID = users.id AND completed = 1), users_admin.userID FROM users LEFT JOIN clan_users ON clan_users.userID = users.id LEFT JOIN clan ON clan.clanID = clan_users.clanID INNER JOIN users_stats ON users_stats.uid = users.id LEFT JOIN ranking_user ON ranking_user.userID = users.id LEFT JOIN users_admin ON users_admin.userID = users.id WHERE users.id = '"+ userID +"' LIMIT 1")
...
for login, premium, clanID, clanName, clanTag, clanOwner, ranking, gameAge, reputation, timePlaying, hackCount, ddosCount, warezSent, spamSent, ipResets, moneyEarned, moneyTransfered, moneyHardware, moneyResearch, profileViews, missionCount, admin in cur.fetchall():	
	...
	if ranking == -1:
		...
		cur.execute("SELECT COUNT(*) AS total FROM ranking_user")
	...
	cur.execute("SELECT COUNT(*) AS total FROM users_friends WHERE userID = '"+userID+"' OR friendID = '"+userID+"'")


	for totalFriends in cur.fetchall():
		totalFriends = totalFriends[0]

	cur.execute("SELECT userID, friendID FROM users_friends WHERE userID = '"+userID+"' OR friendID = '"+userID+"' ORDER BY dateAdd ASC LIMIT 5")
	...
	for friendID1, friendID2 in cur.fetchall():
		...
		cur.execute("SELECT login, cache.reputation, ranking_user.rank, clan.name, clan.clanID FROM users LEFT JOIN cache ON cache.userID = users.id LEFT JOIN ranking_user ON ranking_user.userID = users.id LEFT JOIN clan_users ON clan_users.userID = users.id LEFT JOIN clan ON clan.clanID = clan_users.clanID WHERE users.id = '"+friendID+"' LIMIT 1")
	...
	cur.execute("SELECT users_badge.badgeID, COUNT(users_badge.badgeID) FROM users_badge JOIN badges_users ON badges_users.badgeID = users_badge.badgeID WHERE users_badge.userID = '"+userID+"' GROUP BY users_badge.badgeID ORDER BY badges_users.priority, badges_users.badgeID")
	...
	with open('../profile/' + userID + '_' + lang + '.html', 'w', encoding='utf-8') as f:
		f.write(html)

	cur.execute('INSERT INTO cache_profile (userID, expireDate) VALUES ('+str(userID)+', NOW()) ON DUPLICATE KEY UPDATE expireDate = NOW()')
